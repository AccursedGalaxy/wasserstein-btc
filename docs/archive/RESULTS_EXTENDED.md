> **Archived 2026-09-22 (per `../PREREG.md`, v1.0 deprecations).** Retained for provenance only; not a load-bearing claim. This panel compares `WGeo-GARCH-Ens`, which is not the pre-registered headline forecaster (`WGeo-Ensemble`), and its cross-references to the `RESULTS_LONG.md` baseline panel predate the removal of the duplicate `RW-Drift` baseline. Regenerate with `wbtc extended-baselines` (now writes here).
