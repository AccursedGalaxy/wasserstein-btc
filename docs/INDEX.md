...
## [archive/RESULTS_EXTENDED.md](archive/RESULTS_EXTENDED.md) — named-econometric comparator panel (archived)

**Archived 2026-09-22 per `PREREG.md`; provenance only, not load-bearing.** The v0.4 extended-baselines report. Compares `WGeo-GARCH-Ens` (the v0.3
...
+ conditional coverage, and Acerbi-Szekely (2014) Z1 / Z2 expected-
shortfall tests with Monte-Carlo p-values. Covers α ∈ {1%, 5%} on
BTC + ETH + SOL + BNB + XRP at h ∈ {1, 5}. **The economically
meaningful tail-claim** — CRPS averages over the whole distribution and
tail mis-calibration can be hidden by good body fit; these tests
isolate it. Generated by `wbtc var-es` → `scripts/run_var_es_backtest.py`.

## [archive/RESULTS.md](archive/RESULTS.md) — superseded, archived

**Archived 2026-09-22 per `PREREG.md`; provenance only.** Its `RW-Drift` column is byte-identical to `Static`. The original v0.1 365-day-holdout backtest. Shows -11.7% CRPS vs GARCH-N at
...
`THEORY.md`. When pointing to coverage / calibration evidence, use
`RESULTS_VAR_ES.md`. For academic citation, see
